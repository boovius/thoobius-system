...
type ActionRoots = { stateRoot: string; artifactRoot: string };
...
function assertPageId(value: string): void {
  if (!/^[0-9a-f-]{32,36}$/i.test(value)) throw new Error("Protected NTC action has an invalid page ID");
}

function assertFlowId(value: string): void {
  if (!/^[0-9a-f-]{32,36}$/i.test(value)) throw new Error("Protected NTC action has an invalid flow ID");
}

function assertActionMatchesPolicy(action: NtcProtectedAction, roots: ActionRoots): void {
  if (action.script !== SCRIPT_BY_KIND[action.kind]) throw new Error("Protected NTC action script is not allowlisted");
  ...
  if (action.kind === "read_page") {
    ...
    if (extra.length || outputPath !== expected) throw new Error("Protected NTC page-read arguments are not allowlisted");
    return;
  }
  if (action.kind === "publish_notion") {
    ...
    if (!/^[0-9a-f]{64}$/i.test(action.artifactSha256 ?? "")) throw new Error("Protected NTC publication lacks an artifact hash");
    return;
  }
  const [flowId, stateRoot, ...extra] = action.args;
  assertFlowId(flowId ?? "");
  if (extra.length || stateRoot !== roots.stateRoot) throw new Error("Protected NTC monitor arguments are not allowlisted");
}

function grantPath(actionId: string, suffix = "json"): string {
  if (!/^[0-9a-f]{64}$/i.test(actionId)) throw new Error("Invalid NTC protected action ID");
...
export function issueNtcExecutionGrant(params: {
  ...
  now?: Date;
  ttlMs?: number;
}): { actionId: string; command: string; expiresAt: string } {
  assertFlowId(params.flowId);
  if (!Number.isInteger(params.revision) || params.revision < 0) throw new Error("Invalid NTC flow revision");
  assertActionMatchesPolicy(params.action, params.roots);
  const now = params.now ?? new Date();
  const ttlMs = params.ttlMs ?? DEFAULT_GRANT_TTL_MS;
  if (!Number.isFinite(ttlMs) || ttlMs <= 0 || ttlMs > DEFAULT_GRANT_TTL_MS) throw new Error("Invalid NTC action grant lifetime");
  const actionId = randomBytes(32).toString("hex");
  const expiresAt = new Date(now.getTime() + ttlMs).toISOString();
  const grant: NtcExecutionGrant = {
    version: 1,
    actionId,
...
function parseGrant(value: string): NtcExecutionGrant {
  const grant = JSON.parse(value) as Partial<NtcExecutionGrant>;
  ...
  return grant as NtcExecutionGrant;
}

function defaultRunner(params: { script: string; args: string[]; env: NodeJS.ProcessEnv }) {
...
function sanitizedFailure(run: ReturnType<ActionRunner>): string {
  if (run.error) return run.error.name || "executor_error";
...
export function executeNtcExecutionGrant(params: {
  ...
  if (!existsSync(source)) throw new Error("NTC protected action grant is missing or already consumed");
  renameSync(source, claimed);
  let grant: NtcExecutionGrant;
  try {
    grant = parseGrant(readFileSync(claimed, "utf8"));
    if (grant.actionId !== params.actionId) throw new Error("NTC protected action grant identity mismatch");
    ...
    if ((params.now ?? new Date()).getTime() > Date.parse(grant.expiresAt)) throw new Error("NTC protected action grant expired");
    const action: NtcProtectedAction = {
      kind: grant.kind,
    ...
    if (grant.kind === "publish_notion") {
      ...
      if (actualHash !== grant.artifactSha256) throw new Error("NTC publication artifact changed after authorization");
    }
    if (!process.env.NTC_NOTION_API_KEY) throw new Error("Gateway did not inject the protected NTC Notion credential sentinel");
    const run = (params.runner ?? defaultRunner)({
      script: SCRIPT_BY_KIND[grant.kind],
      args: grant.args,
      env: { ...process.env, OPENCLAW_NOTION_PROFILE: "ntc", NTC_STATE_ROOT: grant.stateRoot },
    });
    const receipt: NtcExecutorReceipt = {
    ...
    if (existsSync(claimed)) unlinkSync(claimed);
  }
}
