...
import { executeNtcExecutionGrant, issueNtcExecutionGrant, type NtcProtectedAction } from "./ntc-protected-executor.js";

const originalCredential = process.env.NTC_NOTION_API_KEY;
const testRoots: string[] = [];
const grantIds: string[] = [];
afterEach(() => {
  if (originalCredential === undefined) delete process.env.NTC_NOTION_API_KEY;
  else process.env.NTC_NOTION_API_KEY = originalCredential;
  ...
  for (const actionId of grantIds.splice(0)) {
    ...
    for (const suffix of ["json", "running", "result.json"]) rmSync(path.join(grantRoot, `${actionId}.${suffix}`), { force: true });
  }
});
